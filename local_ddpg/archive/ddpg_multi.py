...
then train it on the alternating curriculum (random curves + multi-robot
per-segment maps):
...
ROBOT_COLLISION_RADIUS = 0.15   # robot footprint radius (for robot-robot d_obs)
SAFETY_MARGIN = 0.25            # extra buffer added on top of geometric radius
...
HARD_COLLISION_RADIUS = 0.35       # d_obs_min below this ends the episode
...
# ----------------------------------------------------------------------
# 8. Curriculum training loop: alternates random-curve rounds (single
#    robot, no obstacles) and multi-robot map rounds (per-segment,
#    static + dynamic obstacles)
# ----------------------------------------------------------------------

def train(max_rounds, maps, init_from_5dim=None, episodes_per_path=EPISODES_PER_PATH,
          output_dir=OUTPUT_DIR, verbose_every=10, seed=None,
          eval_every_rounds=EVAL_EVERY_ROUNDS, resume_from=None):
    """Trains ONLY on the JSON maps given in `maps` (no random-curve rounds).
    Each round picks one map and runs its robots together (so they see each
    other as dynamic obstacles), cycling through the given maps in order."""
    ...
    map_data = []  # list of dicts: {"map_name", "robots": [{"segments":[...], "obstacles":[...]}]}
    for prefix in maps:
        ...
        # obstacles are shared/global per map; use the union (they're
        # identical per-robot in this dataset, but union covers the general
        # case safely).
    ...
    episode_rewards = []
    episode_kind = []      # map_name
    greedy_eval_log = []
    global_episode = 0

    for round_idx in range(max_rounds):
        chosen_map = map_data[round_idx % len(map_data)]
        per_robot_segment_paths = [robot["segments"] for robot in chosen_map["robots"]]
        static_obstacles = chosen_map["obstacles"]
        kind = chosen_map["map_name"]

        print(f"\n=== Round {round_idx + 1}/{max_rounds} | map = {kind} "
              f"| n_robots = {len(per_robot_segment_paths)} | n_obstacles = {len(static_obstacles)} ===")

        for ep_in_path in range(episodes_per_path):
            global_episode += 1
            env = MultiRobotPathFollowEnv(per_robot_segment_paths, static_obstacles)
            states = env.reset()
            agent.noise.reset()
            episode_reward = 0.0
            all_done = False
            step_count = 0
            max_steps = MAX_STEPS_PER_EPISODE * max(1, max(len(s) for s in per_robot_segment_paths))

            print(f"  -- Episode {global_episode} (path ep {ep_in_path + 1}/{episodes_per_path}) start --")
            ...
            episode_kind.append(kind)

            n_success = sum(env.success_flags)
            print(f"  Episode {global_episode:5d} done | reward={episode_reward:.3f} | "
                  f"robots reached final goal = {n_success}/{env.n_robots} | "
                  f"noise sigma={agent.noise.sigma:.3f}")

        if (round_idx % eval_every_rounds == 0) or (round_idx == max_rounds - 1):
            eval_env = MultiRobotPathFollowEnv(per_robot_segment_paths, static_obstacles)
            eval_states = eval_env.reset()
            eval_all_done = False
            eval_steps = 0
            eval_max_steps = MAX_STEPS_PER_EPISODE * max(1, max(len(s) for s in per_robot_segment_paths))
            while not eval_all_done and eval_steps < eval_max_steps:
            ...
            greedy = {"round": round_idx, "kind": kind, "success": n_success == eval_env.n_robots,
                      "steps": eval_steps, "reward": 0.0, "first_action_mag": None}
            greedy_eval_log.append(greedy)
            eval_env.close()
            print(f"  [greedy eval, no noise] robots succeeded = {n_success}/{eval_env.n_robots} | steps={eval_steps}")

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path = os.path.join(output_dir, f"ddpg_obstacle_multi_robot_{max_rounds}_{timestamp}.pt")
    agent.save(save_path)
    print(f"Saved 7-dim checkpoint to {save_path}")
    plot_training_curve(episode_rewards, episode_kind, output_dir, greedy_eval_log)
    return agent, episode_rewards, episode_kind, greedy_eval_log


def plot_training_curve(episode_rewards, episode_kind, output_dir, greedy_eval_log=None):
    window = 20
    ...
    for i in range(1, len(episode_kind)):
        if episode_kind[i] != episode_kind[i - 1]:
            ax.axvline(i, color="gray", linewidth=0.5, alpha=0.4)

    if greedy_eval_log:
        episodes_per_round = max(1, len(episode_rewards) / max(1, (max(e["round"] for e in greedy_eval_log) + 1)))
        xs = [int((e["round"] + 1) * episodes_per_round) - 1 for e in greedy_eval_log]
        ys = [episode_rewards[min(x, len(episode_rewards) - 1)] for x in xs]
        colors = ["green" if e["success"] else "red" for e in greedy_eval_log]
        ax.scatter(xs, ys, c=colors, marker="D", s=60, zorder=5,
                   label="Greedy (no-noise) eval\n(green=all robots succeeded, red=did not)")

    ax.set_xlabel("Episode")
    ax.set_ylabel("Total reward")
    ax.set_title("Obstacle-Aware DDPG Training Reward (7-dim, single+multi-robot curriculum)")
    ax.legend()
    ax.grid(alpha=0.3)
    plt.tight_layout()
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
...
if __name__ == "__main__":
    ...
    # python ddpg_obstacle_multi_robot.py --init_from_5dim solves_drl/ddpg_path_following_multi_curve_60_XXXX.pt --maps map_003 --max_rounds 80

    # python ddpg_multi.py --init_from_5dim solves_drl/ddpg_path_following_multi_curve_60_XXXX.pt --maps map_003 --max_rounds 80