...
========================================================================
WAYPOINT-BASED VERSION (Option 1) - TOP-20% WARM START + GENERATION LOGGING
----------------------------------------------------------------------------
Each segment between two task waypoints is subdivided into 6 free
intermediate points (12 DE variables: x,y for each point). The path
within a segment is the straight-line polyline through:
seg_start -> p1 -> p2 -> ... -> p6 -> seg_end.

Changes vs the earlier version:
  1. Warm-start now seeds the next segment's initial population from the
     TOP 20% (by cost) of the PREVIOUS segment's final population, not
     just the single best solution.
  2. A per-generation callback prints the best cost at every generation.
  3. The exported path JSON now also includes the actual (x, y)
     coordinates of the 6 intermediate/free points for each segment.
"""
...
WARM_START_FRACTION = 0.5      # fraction of new population seeded from elite pool
WARM_START_NOISE_STD = 0.65
WARM_START_TOP_PCT = 0.20      # keep top 20% of previous segment's final population
...
# ----------------------------------------------------------------------
# 5. Differential Evolution (Top-20% Warm Start + Generation Callback)
# ----------------------------------------------------------------------
...
def build_warm_start_population(prev_best_pop, popsize, n_vars):
    """
    prev_best_pop: array of elite individuals (top 20%) from the PREVIOUS
    segment's final DE population, or None for the first segment.
    """
    ...
    if prev_best_pop is not None and len(prev_best_pop) > 0:
        for _ in range(n_seeded):
            ...
            jittered = base + np.random.normal(0.0, WARM_START_NOISE_STD, size=n_vars)
...
def solve_sequential():
    all_delta_vectors = []
    prev_best_pop = None   # top-20% elite population from previous segment
    total_cost = 0.0
    popsize = 50
    ...
    for seg in range(N_SEGMENTS):
        ...
        # --- extract top 20% of the FINAL population as elites for next segment ---
        final_pop_scaled = solver._scale_parameters(solver.population)   # actual param values
        final_costs = solver.population_energies
        n_top = max(1, int(WARM_START_TOP_PCT * len(final_pop_scaled)))
        top_idx = np.argsort(final_costs)[:n_top]
        prev_best_pop = final_pop_scaled[top_idx]

        print(f"  Segment {seg + 1}/{N_SEGMENTS} done: cost={result.fun:.4f}, nfev={result.nfev}, "
              f"elites_kept={n_top}")
...
def export_path(curves, delta_vectors, filename="planned_path.json"):
    ...
    for seg in range(N_SEGMENTS):
        seg_start = WAYPOINTS[seg]
        seg_end = WAYPOINTS[seg + 1]
        free_points = delta_to_points(delta_vectors[seg], seg_start, seg_end)

        segments_data.append({
            "segment_index": seg,
            "start": seg_start.tolist(),
            "end": seg_end.tolist(),
            "intermediate_points": free_points.tolist(),   # the 6 free (x,y) points
            "dense_curve": curves[seg].tolist()            # full sampled polyline
        })
    ...
    print(f"Saved optimized path (with intermediate point coordinates) to {filename}")
...
def plot_result(delta_vectors, total_cost):
    ...
    ax.set_aspect("equal")
    ax.set_title(f"Sequential DE over Waypoint Deltas (6 pts/seg, top-20% warm start, cost={total_cost:.3f})")
    ax.grid(alpha=0.3)
    plt.tight_layout()
...
def run_planner(input_map_json, output_path_json="planned_path.json"):
    ...
    print(f"Running sequential per-segment DE over {N_FREE_POINTS}-point waypoint deltas "
          f"({N_VARS_PER_SEGMENT} DE variables/segment), top-{int(WARM_START_TOP_PCT*100)}% warm start...")
...
if __name__ == "__main__":
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_planner("maps/env_map_config_028.json", f"solves/planned_path_{timestamp}.json")
