...
N_SAMPLES_PER_SEGMENT = 80         # samples drawn along each segment's curve
...
CHECKPOINT_EVERY_N_GENERATIONS = DE_MAXITER  # save intermediate solution state every N generations
...
def build_delta_bounds(seg_start, seg_end):
    ...
    delta_bound = max(5, 0.4 * segment_length)
...
if __name__ == "__main__":
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_planner("maps/env_map_config_011.json", f"solves/planned_path_{timestamp}.json")
