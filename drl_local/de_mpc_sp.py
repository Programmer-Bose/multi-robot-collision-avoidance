...
POPSIZE = 15                # scipy DE popsize multiplier -> pop = popsize * n_params
N_GENERATIONS = 8           # maxiter (lower now that warm-starting speeds convergence)
DT = cu.SIM_DT
...
def de_optimize(state, polyline, cum_arclen, s_prev, obstacles, other_positions, rng):
    ...
    _, s_now = evaluate_population(
        best_seq[None, :, :], state, polyline, cum_arclen, s_prev, obstacles, other_positions
    )
