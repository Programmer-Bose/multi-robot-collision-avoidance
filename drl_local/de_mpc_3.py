...
W_TARGET = 12.0                  # pulls robot toward its current sequential target (task point or goal)
TARGET_REACH_RADIUS = 0.3        # world units - counts a task point/goal as "visited"


class RobotState:
    __slots__ = ("pos", "heading", "prev_action", "prev_seq", "target_idx")
    ...
    def __init__(self, pos, heading):
        ...
        self.target_idx = 0   # index into the robot's [task_pt_1, task_pt_2, ..., goal] sequence
...
def evaluate_population(pop, state, polyline, cum_arclen, s_prev, obstacle_circles, other_positions, current_target):
    ...
    # --- pull toward the current sequential target (next unvisited task point, then goal) ---
    target_dist = np.hypot(traj_pos[:, -1, 0] - current_target[0], traj_pos[:, -1, 1] - current_target[1])
    target_pen = W_TARGET * target_dist
...
def de_optimize(state, polyline, cum_arclen, s_prev, obstacle_circles, other_positions, current_target, rng):
    ...
    def objective(x):
        ...
        cost, _ = evaluate_population(pop, state, polyline, cum_arclen, s_prev, obstacle_circles,
                                       other_positions, current_target)
        return cost
    ...
    _, s_now = evaluate_population(
        best_seq[None, :, :], state, polyline, cum_arclen, s_prev, obstacle_circles,
        other_positions, current_target
    )
...
class DEMPCController:
    ...
    def __init__(self, map_json_path, path_json_paths, n_robots=None, seed=0, render=False):
        ...
        # --- sequential targets: task_points visited in task_sequence order, then the goal ---
        task_points = self.map_data["task_points"]
        task_sequence = self.map_data["task_sequence"]
        goal = self.map_data["goal"] if self.map_data["goal"] is not None else self.polylines[0][-1]
        self.targets = [task_points[tid] for tid in task_sequence] + [goal]
    ...
    def render(self):
        ...
        for tid, pt in self.map_data["task_points"].items():
            pygame.draw.circle(screen, (30, 120, 220), self._world_to_screen(pt), 8, 2)

        pygame.draw.rect(screen, cu.RENDER_COLOR_HUD_BG,
                          (0, cu.RENDER_SCREEN_SIZE, cu.RENDER_SCREEN_SIZE, cu.RENDER_PANEL_HEIGHT))
        progress_str = " ".join(f"r{i}:{s.target_idx}/{len(self.targets) - 1}" for i, s in enumerate(self.states))
        hud_text = f"step {self.step_count}  n_robots={self.n_robots}  DE-MPC  {progress_str}"
    ...
    def step(self):
        ...
        for i, state in enumerate(self.states):
            ...
            current_target = self.targets[min(state.target_idx, len(self.targets) - 1)]
            action, s_now = de_optimize(
                state, self.polylines[i], self.arclens[i], self.s_prev[i],
                self.obstacle_circles, others, current_target, self.rng
            )
            ...
            # advance to the next sequential target once the current one is reached
            if state.target_idx < len(self.targets) - 1:
                if float(np.linalg.norm(current_target - state.pos)) <= TARGET_REACH_RADIUS:
                    state.target_idx += 1
    ...
    def goal_reached(self, i):
        ...
        state = self.states[i]
        sequence_done = state.target_idx >= len(self.targets) - 1
        return sequence_done and float(np.linalg.norm(goal - state.pos)) <= cu.GOAL_REACH_RADIUS
